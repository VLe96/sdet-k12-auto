...
public class Component {
    protected WebDriver driver; // để component này trở thành class cha của các page object khác, dc sd lại
    protected WebElement component; // đại diện cho component tìm dc.
    ...
    public <T extends Component> T findComponent(Class<T> componentClass){ //nghĩa là các page components phải extend
        // cái này ra khi sd component này
    ...
    public <T extends Component> List<T> findComponents(Class<T> componentClass){
        ...
        try{
            ...
            throw new IllegalArgumentException("[ERROR] The component must have annotation for: " +
                    "CSSSelector, ID...");
        }
        List<WebElement> results = wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(componentSelector));

        // Define component's contructor
        ...
        try{
            ...
            throw new IllegalArgumentException("[ERR] The component must have a construtor"
            + Arrays.toString(params));
        }
        // chuyển list webelement sang list component
    ...
    private By getComponentSelector(Class<? extends Component> componentClass){
        if(componentClass.isAnnotationPresent(ComponentCSSSelector.class)){
            ...
            return By.cssSelector(componentClass.getAnnotation(ComponentIDSelector.class).value());
        }else {
